...
public class VATSetupStatic : MonoBehaviour
{

    [SerializeField] private MeshRenderer staticMeshRenderer;
    ...
    private void Awake()
    {
        vat = new VATStatic(meshRenderer, animator, animationClip);
        rt = vat.WriteToVAT();
        SaveTexture(rt, vat.textureWidth, vat.textureWidth);

        mesh = MeshExtensions.CopyMesh(meshRenderer.sharedMesh);
